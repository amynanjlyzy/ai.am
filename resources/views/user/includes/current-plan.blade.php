        ...
        @foreach ($activeFeatureLimits as $key => $activeFeaturelimit)
            ...
            @else
                ...
                <div
                    ...
                    <p>{{ __('Credits Used') }}:
                        @if ($activeFeaturelimit['limit'] == -1)
                            {{ $activeFeaturelimit['used'] }}/{{ in_array($activeSubscription->status, ['Active', 'Cancel']) ? __('Unlimited') : '0' }}</p>
                        @else
                            {{ $activeFeaturelimit['used'] }}/{{ in_array($activeSubscription->status, ['Active', 'Cancel']) ? $activeFeaturelimit['limit'] : '0' }}</p>
                        @endif
