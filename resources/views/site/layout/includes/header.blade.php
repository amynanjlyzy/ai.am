...
<header id="top" class="relative pt-scroll md:pt-[74px] pt-14">
    <nav id="top-menu"
        ...
        <div class="flex justify-between">
            ...
            @endif
            <button id="menuBtn" class="block md:hidden focus:outline-none" type="button" onclick="navToggle();">
            ...
            <div id="dropdowns-header" class="md:relative absolute top-0 right-[50px] md:right-1 flex justify-end items-center lg:gap-5 gap-3">
                @if (isset($header['main']['show_switch_bar']) && $header['main']['show_switch_bar'] == 1)
                    <div class="mt-3 md:mt-0 cursor-pointer" id="switch">
                        <img  src="{{ asset('Modules/OpenAI/Resources/assets/image/moon.png') }}" class="moon w-6 {{ \Cookie::get('theme_preference') == 'dark' ? 'hidden' : '' }}" alt="moon">
                        <img src="{{ asset('Modules/OpenAI/Resources/assets/image/sun.png') }}" class="sun w-6 {{ \Cookie::get('theme_preference') == 'dark' ? '' : 'hidden' }}" alt="sun">
                    </div>
                @endif
                <div>
                    @php
